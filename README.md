Copy karke README.md me paste kar do:
...
1. **Load** — `PIL.Image.open()` reads PNG/JPG images.
2. **Greyscale** — `.convert("L")` converts RGB to a single luminance channel.
3. **Resize** — `.resize((64, 64))` standardizes image dimensions.
4. **Flatten** — `numpy.flatten()` transforms the image into a 1-D feature vector.
5. **Standardise** — `StandardScaler` scales features to zero mean and unit variance.
